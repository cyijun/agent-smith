...
本示例展示如何使用 Agent Smith 框架进行复杂的 AI Agent 市场研究任务。
...
这是一个典型的需要多维度分析的复杂任务，非常适合使用 Agent Smith 的分解-并行-汇总模式。
...
Agent Smith 分析任务特点后，首先通过 **Red Flags 检查**确认分解的合理性：

- [x] 任务复杂度远超 3 个主要步骤（涉及数据收集、分析、交叉验证、报告撰写）
- [x] 各维度可独立并行，无强耦合
- [x] 当前层级 = 0 < 3，满足层级约束
- [x] 分解为 4 个子任务 < 5，满足数量约束
- [x] 每个子任务预计耗时 3-4 天，远超 15 分钟红线

**通过检查**，决定将其分解为 4 个并行的子任务：
...
根史密斯在汇总完所有子结果并写入自身 `outbox/result.md` 后，执行最终职责：

```
cp outbox/result.md .agent-smith/results/final.md
```

`final.md` 作为用户获取完整研究报告的统一入口，内容应与根史密斯的 `outbox/result.md` 保持一致。
...
本示例展示了 Agent Smith 如何处理复杂的市场研究任务：
