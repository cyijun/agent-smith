...
**史密斯 (Smith)** 是 Agent Smith 中的基本智能体单元，是一个递归自相似的多智能体节点。每个史密斯都是一个完整的、能够独立工作的 AI 实体，具有唯一的标识符（如 `smith-root`、`smith-001`）和明确的层级关系。

史密斯的命名灵感来源于《黑客帝国》中的特工史密斯——能够自我复制、分布式的存在。在系统中，每个史密斯都遵循相同的核心协议，具备一致的行为模式。

### 1.2 自相似特性

自相似是 Agent Smith 的核心设计哲学，体现在以下方面：

**结构自相似**：每个史密斯拥有相同的目录结构（`inbox/`、`private/`、`outbox/`、`children/`），无论层级深浅，结构保持一致。

**行为自相似**：所有史密斯使用完全相同的协议和决策逻辑。根史密斯分解任务的策略，与子史密斯分解子任务的策略完全一致。

**协议自相似**：读写规则、通信机制、结果格式在所有层级统一适用。

这种自相似性使得系统具有极强的可扩展性和可预测性——理解一个史密斯，就等于理解了整个系统。
...
目录隔离是 Agent Smith 实现无冲突并行的基础。每个史密斯拥有独立的工作空间，通过严格的写权限控制避免资源竞争。

### 2.1 inbox/ —— 任务队列

**作用**：存放父史密斯分配的任务，是父史密斯向子史密斯分配工作的唯一通道。

**特点**：
- **父写子读**：只有父史密斯能在子史密斯的 inbox/ 中创建任务文件，子史密斯只能读取自己的 inbox/
- 任务文件命名规范：`task-{SMITH_ID}.md`
- 任务被读取后即视为"处理中"
...
**特点**：
- **父管生原则**：只有父史密斯能创建子目录
- 每个子目录是一个完整的史密斯环境，包含 `inbox/`、`private/`、`outbox/`、`children/`
- 子目录内部同样包含 `inbox/`（父写入任务）、`private/`、`outbox/`、`children/`

**创建流程**：
1. 父史密斯在 `children/` 下创建子史密斯目录（包含 inbox/）
2. 父史密斯在子史密斯的 `inbox/` 中创建任务文件
3. 父史密斯完成子目录创建，子史密斯就绪（最大层级3，层级≥3时禁止分解）
4. 子史密斯读取自己的 `inbox/` 中的任务，开始执行
...
是 → 设计子任务（确定数量、定义边界、分配ID）
    ↓
创建 children/smith-{子ID}/ 目录结构
    ↓
在子史密斯的 inbox/ 中创建 task-{子ID}.md 文件
    ↓
...
### 3.4 约束门控（Constraint Gates）

在创建任何子史密斯之前，父史密斯**必须**执行以下强制检查。若任一检查未通过，则**禁止**分解，必须直接执行任务：

- [ ] 当前层级 < 3（若层级 ≥ 3，停止——直接执行）
- [ ] 现有子代理数量 < 5（若已达 5 个，停止——直接执行或重新分解）

约束门控是防止系统资源耗尽和过度分解的最后一道防线。违反门控创建的子史密斯不受协议保护，其输出视为无效。
...
- **叶节点史密斯**：直接完成任务，输出结果到 `outbox/result.md`
- **中间节点史密斯**：所有子任务完成并汇总后，输出结果到 `outbox/result.md`
- **根史密斯**：汇总所有子结果后，输出到 `outbox/result.md`，并**额外**将结果复制或汇总到 `.agent-smith/results/final.md`

根史密斯的 `final.md` 职责是用户获取完整输出的统一入口。即使 `outbox/result.md` 已包含完整内容，也应确保 `final.md` 存在且一致。
