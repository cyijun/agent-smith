...
本文档总结了在使用 Agent Smith 进行任务分解和并行处理时的最佳实践，帮助用户获得更高效、更可靠的执行结果。

---

## 一、任务分解判断标准

正确的任务分解是 Agent Smith 高效运作的关键。以下三个标准帮助判断何时应该分解任务：
...
过度分解是新手使用 Agent Smith 时最常见的错误。递归必须有终止条件。
...
- **汇总困难**：合并结果时需要大量重写和重构
- **深度过深**：递归层级超过3层（已达协议硬限制）

### 3.3 Red Flags —— 停止分解的警戒线

遇到以下任何情况时，**禁止**继续分解，应直接执行：

- **子任务预计耗时少于 15 分钟**：分解带来的协调开销会超过收益
- **当前层级已 ≥ 3**：已达协议硬限制，禁止创建子史密斯
- **同层子代理数量将达到或超过 5 个**：已达每层最大子代理数限制
- **子任务之间耦合紧密或需要频繁同步**：违背并行分解的初衷

这些 Red Flags 与 [Hard Constraints](../SKILL.md#hard-constraints-quick-reference) 一致，是协议强制执行的底线，不可突破。
...
该文件作为用户获取多智能体会话完整输出的统一入口。即使根史密斯的 `outbox/result.md` 已包含完整结果，也应确保 `final.md` 存在且内容一致，以便用户按约定路径获取最终交付物。

---

## 五、总结

Agent Smith 的最佳实践可以概括为：
