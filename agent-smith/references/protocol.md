...
无冲突协议 (Conflict-Free Protocol) 是 Agent Smith 实现并行安全的核心机制。通过严格的目录隔离和写权限控制，确保多个史密斯同时工作时不会产生资源竞争或数据冲突。
...
**每个史密斯只能写入自己的私有空间，以及自己创建的子史密斯的 inbox。** 这是无冲突协议的第一准则，从根本上杜绝了写冲突的可能性。
...
| `children/{子ID}/inbox/` | 子任务队列 | 为子史密斯创建任务文件（父权限） |
...
- **其他史密斯的 `inbox/`**：禁止修改已分配的任务
- **父史密斯的任何目录**：子史密斯只能读取父的输出，不能反向写入
- **根级别的 `.agent-smith/results/final.md`**：只有根史密斯能写入最终结果
...
1. **创建子任务**：在自己创建的子史密斯目录的 `inbox/` 中写入任务文件
...
父史密斯行使创建权限前，**必须**通过以下强制检查。任一检查未通过，则禁止创建子史密斯，必须直接执行任务：
...
每个史密斯的 `inbox/` 目录作为该史密斯的任务队列，存放父史密斯分配的任务文件：
...
1. **任务创建**：父史密斯在子史密斯的 `inbox/` 中创建 `task-{ID}.md` 文件
2. **任务读取**：子史密斯启动后读取自己的 `inbox/` 中的任务文件
...
当创建子史密斯时，父史密斯将 `agent-smith/smith.md` **只读复制**到子目录：
...
本协议是基于文件系统约定的协作框架，不强制限制写操作。协议遵守依赖各史密斯的自觉遵守。在自动化环境中，建议配合以下机制：
...
这些机制相互配合，构成了 Agent Smith 的并发安全基础，使得大规模并行任务分解成为可能。
